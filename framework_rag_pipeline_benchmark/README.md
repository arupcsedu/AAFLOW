...
- `fair_parallelism_plus_overlap` for overlap-oriented comparisons where `AgenticDRC` keeps a native overlapping pipeline and non-Agentic frameworks incur modeled dispatch overhead per batch.
...
- `AgenticDRC` uses native overlap between generation and streaming `embed -> upsert`
- non-Agentic frameworks execute the same stages with modeled per-batch dispatch overhead in `fair_parallelism_plus_overlap`

This is the profile used when the goal is to demonstrate a large `AgenticDRC` advantage.
...
Observed summary:
- `AgenticDRC`: `total_s=3.380`, `upsert_s=1.848`, `embed_s=0.276`
...
Observed summary:
- `AgenticDRC`: `total_s=0.916`, `load_s=0.024`, `transform_s=0.015`, `embed_s=0.492`, `upsert_s=0.072`
...
- `AgenticDRC` remains the fastest framework in the distributed FAISS overlap profile.
